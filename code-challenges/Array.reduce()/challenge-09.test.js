...
const reversedString = (str) => {
  ...
  return newArr.reduce((newStr, letter) => {
...
const countNumberOfChildren = (arr) => {
  const result = arr.reduce((kidsCount, character) => {
    if (character.children !== undefined) {
      console.log(kidsCount);
      kidsCount += character.children.length;
    }
    return kidsCount;
  }, 0)
  return result;
