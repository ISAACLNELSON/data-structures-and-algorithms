...
const count = (num, arrOarrs) => {
    //create a counter
    let counter = 0;
    //iterate over all arrays in arrOarrs
    arrOarrs.forEach(arr => {
        if (arr.includes(num)) {
            arr.forEach(ele => ele === num ? counter++ : undefined)
        }
    })
    //if current element = num => counter++
...
const divisibleByFiveTwoToThePower = (input) => {
    ...
    const container = [];
    //make a new innerArr 
    const newArr = input.forEach(arr => {
        arr.filter(num => {
            if (typeof num === 'number' || num % 5 === 0) {
                return Math.pow(2, num);
            }
        })
        container.push(newArr);
    })
    return container;
};
...
data.map(obj => {
    if(obj.name.length <= shortest.length){
        shortest = obj.name;
    }
})
return shortest;
