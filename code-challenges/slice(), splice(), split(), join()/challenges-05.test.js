...
const listFoods = (recipe) => {
  let result = [];
  let currentItem = recipe.ingredients;
  //iterate over ingredients arr in gruffaloCrumble 
  for (let i = 0; i < recipe.ingredients.length; i++) {
    if (currentItem[i].includes('8 pounds') ||
      currentItem[i].includes('2 pounds') ||
      currentItem[i].includes('4 pounds')
      // currentItem[i].includes('1 medium-sized') ||
      // currentItem.includes('2 gallons') ||
      // 
      // currentItem.includes('6 gallons')
    ) {
      result.push(currentItem[i].slice(9));
    }

    if (currentItem[i].includes('16 cups') ||
      currentItem[i].includes('1 pound')) {
      result.push(currentItem[i].slice(8));
    }

    if (currentItem[i].includes('2 gallons') ||
      currentItem[i].includes('6 gallons')) {
      result.push(currentItem[i].slice(10));
    }

    if (currentItem[i].includes('1 medium-sized')) {
      result.push(currentItem[i].slice(15));
    }
  }
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  let splitUpArr = [];
  let currentItem = recipe.ingredients;
  // split the trash out of each array and push the rest?
  for (let i = 0; i < currentItem.length; i++) {
    splitUpArr.push(currentItem[i].split(' '));
    // splitUpArr.push(currentItem[i].split('pounds'))
    // splitUpArr.push(currentItem[i].split('pound'))
    // splitUpArr.push(currentItem[i].split('medium-sized'))
    // splitUpArr.push(currentItem[i].split('cups'))
  }

  for (let i = 0; i < splitUpArr.length; i++) {
    if (currentItem[i].includes('8 pounds') ||
      currentItem[i].includes('2 pounds') ||
      currentItem[i].includes('4 pounds')
    ) {
...
const stepActions = (recipe) => {
  ...
  for (let i = 0; i < recipe.steps.length; i++) {
    newArr.push(recipe.steps[i].split(' '));
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  for(let i = str.length; i > str.length - numberOfCharacters - 1; i--){
    newArr.splice(i, 1);
  }
